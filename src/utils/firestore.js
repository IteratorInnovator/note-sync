...
export const getVideosByUserId = async (uid) => {
  const q = query(collection(db, "users", uid, "videos"), orderBy("addedAt", "desc"));
...
export const createNote = async (uid, videoId, content, timeSec) =>
  (await addDoc(collection(db, "users", uid, "videos", videoId, "notes"), { timeSec, content })).id;

export const updateNote = async (uid, videoId, noteId, newContent) => {
  await updateDoc(doc(db, "users", uid, "videos", videoId, "notes", noteId), { content: newContent });
  return noteId;
};
...
export const hasNotes = async (uid, videoId) =>
  !(await getDocs(query(collection(db, "users", uid, "videos", videoId, "notes"), limit(1)))).empty;
...
export const getPlaylistsByUserId = async (uid) => {
  const snap = await getDocs(query(collection(db, "users", uid, "playlists"), orderBy("createdAt", "desc")));
  return snap.docs.map((doc) => ({ playlistId: doc.id, ...doc.data() }));
};

export const createPlaylist = async (uid, title, description = "") =>
  (await addDoc(collection(db, "users", uid, "playlists"), { title, description, createdAt: serverTimestamp() })).id;
